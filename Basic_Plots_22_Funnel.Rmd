...
- **`x` is used as series, while `series` is used to produce separate funnels!**
...
+--------+--------------------------------------------------------------------+
| series | series variable which will be coerced to factors. Each level of `series` is treated as a subsetting factor to produce separate funnels. Only the first one is accepted if multiple variables are provided. |
+--------+--------------------------------------------------------------------+
...
Unlike charts under Cartesian coordinate system, funnel chart uses `series` as subsetting factor to produce seperate polar systems. So when we apply `am` (containing 2 levels) as `series`, we get 2 funnels.
