...
- 数据地图模式:
  ...
  - series映射为独立的地图，即，series的每个水平对应于一副地图。series必须为有效的地理名称
- 可额外向echarts对象添加地理数据
...
|**data**| 数据框格式的源数据    |
+--------+--------------------------------------------------------------------+
| **x**  | 文本型自变量。`x[1,]`必须是有效地理名称，`x[,2]`则被处理为数据系列。如提供多个变量，只传入前两个。 |
+--------+--------------------------------------------------------------------+
...
如设置`series`和`subtype`（有效的地理名称），可得到独立的地图。
