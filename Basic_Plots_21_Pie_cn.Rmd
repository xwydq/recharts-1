...
- **`x`用作数据系列，`series`产生自外而内排布的圈层，`facet`用于产生多个不同的饼图!**
- 饼图和漏斗图可通过工具箱按钮快速切换
...
+--------+--------------------------------------------------------------------+
| series | 转为因子后计算。`series`的每个水平被处理为分组因子，由外向内产生圈层。如提供多个变量，只传入第一个。|
+--------+--------------------------------------------------------------------+
| facet  | 转为因子后计算。`facet`的每个水平被处理为分组因子，用于产生独立的饼图。如提供多个变量，只传入第一个。|
+--------+--------------------------------------------------------------------+
...
饼图用`facet`作为区分不同极坐标系的分组变量。所以当我们把`Class` (有4个水平) 作为`facet`，可以得到4个饼图。
...
如果只提供`facet`和`y`会怎么样？我们会得到多个两分类饼图。
...
### 多层饼图 Multi-ring Pies

如果提供`series`，则生成多层饼图。type和subtype整理成vector，与series各水平映射。设为'ring'，则构成环环相套的多层饼图。
