...
aq1 <- melt(airquality, id=c('Month', 'Day'), measure=c('Temp', 'Wind'), 
            variable='Param')
g <- echartr(aq1, Day, value, Param, z=Month, type='curve') %>% 
...
```{r}
g1 <- g1 %>% relocDataZoom(x=120, y=JS(paste0(
    "document.getElementById('", g1$elementId, "').offsetHeight-30")))
g1
```
...
```{r}
g1 <- g1 %>% relocTimeline(y2=25, x=120, x2=50)
g1
```
...
```{r}
g1 %>% setGrid(x=120, y=40, x2=50, y2=90)
```
