---
title: "recharts: 百度ECharts的R语言接口"
output: 
  html_document: 
    toc: yes
    toc_depth: 4
...
<strong>注意:</strong>本加载包仍在开发完善中。recharts基于百度Echarts2的最后一个稳定发布版(v2.2.7)开发。本文档始终反映**recharts**最新的特性([Github](https://github.com/madlogos/recharts))。安装方法：
...
|           |              |
|           | - 直角坐标系里，`x`与x轴关联 |
|           | - 在极坐标系，`x`的第一变量与极坐标关联，`x`的第二变量为数据系列(映射到图例) |
|           | - 其他类型中，单坐标系可参考直角坐标系的例子，而多坐标系可参考极坐标系的例子。  |
+-----------+------------------------------------------------------------------+
|  y        | 应变量，`data`的一列或多列。始终为数值型。 |
+-----------+------------------------------------------------------------------+
|  series   | 分组变量，`data`的某一列。进行运算时被视作因子。 |
|           |    |
|           | - 直角坐标系里，`series`为数据系列 (映射到图例)。   |
|           | - 极坐标系里，`series`映射到极坐标系的索引值。 |
|           | - 其他类型中，单坐标系可参考直角坐标系的例子，而多坐标系可参考极坐标系的例子。 |
+-----------+------------------------------------------------------------------+
|  weight   | 权重变量，在气泡图、线图、柱图中与图形大小关联。 |
+-----------+------------------------------------------------------------------+
|  z        | 时间轴变量。一旦指定`z`变量，就会生成时间轴组件。 |
+-----------+------------------------------------------------------------------+
|  lat      | 纬度，用于地图/热力图 |
+-----------+------------------------------------------------------------------+
|  lng      | 经度，用于地图/热力图 |
+-----------+------------------------------------------------------------------+
|  type     | 图类型，默认为'auto'。`type`作为向量传入时，映射到`series`向量。 |
+-----------+------------------------------------------------------------------+
| subtype   | 图亚类，默认为NULL。`subtype`作为向量传入时，映射到`series`向量。 |
+-----------+------------------------------------------------------------------+
...
用到时间轴(`z`)时，务必格外小心。`x`、`y`、`series`、`weight`变量一旦用到，就要确保每个`z`水平上，这些变量都完备地排列组合了。否则你会发现各组图形在时间轴上莫名其妙地重叠呈现。

对`series`要特别留意，因为在单坐标系（如直角坐标系）和多坐标系（如极坐标系）中，series的作用是不同的。在前一种情形下，`series`指数据系列。但在后一种情形下，`series`指的是坐标系索引，而`x[,2]`才指数据系列。这样设计是因为`type`和`subtype`固定映射到`series`向量，而在多坐标系图中，用户通常倾向于把不同的图类/亚类映射到多个不同的图，而不是数据系列。
...
修改图例(柠檬色)，初始化时只选中第一系列('Automatic')。
...
通常我们不希望R用户写那么长的选项列表。R用户一般更熟悉（也更常用到）诸如数据框、矩阵、表格等数据结构，理想状态下，我们希望用户只要传入一些熟悉的数据对象，recharts自动地配置出ECharts对象来。目前离理想还有距离。但欢迎反馈您的体验和看法，也欢迎用[Github pull requests](https://github.com/madlogos/recharts)贡献您的代码。
