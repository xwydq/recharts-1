...
1. [混合图类](Misc_03-Mixed_Plots_cn.html)
1. 坐标轴/控件配置
    1. 调整数据系列
        1. [设置系列`setSeries`](Misc_02-setSeries_cn.html)
    1. 坐标轴/制图区
    ...
    1. 样式特性
        1. [样式`aesStyle`](Misc_01-aesStyle_cn.html)
1. 美工
...
1. [定制化](Misc_04-Cust_Plots_cn.html)
...
> 通过`browseVignettes("recharts")`可以离线查看本手册。
...
数据必须以数据框形式传入。数据系列和时间轴会以`series`和`t`变量的原始顺序排序。所以作图前，必须先对数据框进行相应的排序，以确保作出的图里，图例和时间轴以正确的顺序显示。

用到时间轴(`t`)时，务必格外小心。`x`、`y`、`series`、`weight`变量一旦用到，就要确保每个`t`水平上，这些变量都完备地排列组合了。否则你会发现各组图形在时间轴上莫名其妙地重叠呈现。
...
系列/分面 | 分面**1** | 分面**2**  | ... | 分面**i**
---------+------+-------+-------+------------
系列_1_  | list[[**1**]][_1_] | list[[**2**]][_1_] | ... | list[[**i**]][_1_]
系列_2_  | list[[**1**]][_2_] | list[[**2**]][_2_] | ... | list[[**i**]][_2_]
... | ... | ... | ... | ...
系列_j_  | list[[**1**]][_j_] | list[[**2**]][_j_] | ... | list[[**i**]][_j_]
...
如果你想直接修改数据系列的定义，可以调用低级函数`setSeries`。
...
也可以将Echarts用于Shiny。具体可参考`renderEchart`函数。
